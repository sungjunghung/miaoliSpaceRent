<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      <input v-model="search" type="text" class="input" placeholder="搜尋姓名或信箱" />
    ...
    <div class="card bg-base-100 border border-base-200 shadow-sm overflow-x-auto">
      <table class="table">
        ...
        <tbody>
          <tr v-for="admin in filtered" :key="admin.id" class="hover">
            <td>
            ...
            <td>
              <span class="badge">{{ getGroupName(admin.permissionGroupId) }}</span>
            </td>
            <td class="max-w-48 align-top py-3">
              <div v-if="!admin.managedVenues || admin.managedVenues.length === 0" class="text-base-content/50">—</div>
            ...
            <td>
              <div class="flex gap-1">
                <button class="btn btn-ghost btn-square tooltip" title="編輯" data-tip="編輯" @click="openDrawer(admin.id)">
                  <span class="material-symbols-outlined">edit</span>
                </button>
                <button class="btn btn-error btn-ghost btn-square tooltip" title="刪除" data-tip="刪除"
                  :disabled="admin.permissionGroupId === 'role-001'">
